...
export default function Fermat() {
  ...
  async function run() {
    ...
    try {
      ...
      setStatus({ tool: 'fermat', elapsed_ms: elapsed, summary: `${num} → ${verdict}` })
    } catch (e) {
      setError(String(e))
    } finally {
      setLoading(false)
  ...
  return (
    <ToolShell
      ...
      description="Probabilistic test based on Fermat's little theorem: if p is prime, aᵖ⁻¹ ≡ 1 (mod p). Note: susceptible to Carmichael numbers."
    >
      <div className="">
        <div>
        ...
        <ResultCard
          ...
          extra={
            result ? { iterations: result.iterations, 'false-positive': `≤ (1/2)^${result.iterations}` } : undefined
          }
        />
      </div>
    </ToolShell>
